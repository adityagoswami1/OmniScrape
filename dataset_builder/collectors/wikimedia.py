...
class WikimediaCollector(BaseCollector):
    def search_and_download(self, topic):
        ...
        search_url = f"https://commons.wikimedia.org/w/index.php?search={encoded_topic}&title=Special:MediaSearch&go=Go&type=image"
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                print(f"  Warning: Initial Wikimedia load failed or timed out: {e}")
                ...
                try:
                    ...
                    for img in images:
                        ...
                        if src and 'upload.wikimedia.org' in src:
                            # Wikimedia thumbnails look like:
                            # https://upload.wikimedia.org/wikipedia/commons/thumb/a/a2/...jpg/320px-...jpg
                            # We can scrape the thumbnail, or try to reconstruct the original.
                            # For scraping massive datasets, the 320px/800px thumb is often acceptable
                            # and faster. We will collect the src directly.
                            image_urls.add(src)
        ...
        for url in list(image_urls)[:self.max_images]:
            filepath = self.download_image(url, "wikimedia")
            if filepath:
                yield filepath, url

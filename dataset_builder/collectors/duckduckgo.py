...
class DuckDuckGoCollector(BaseCollector):
    def search_and_download(self, topic):
        ...
        search_url = f"https://duckduckgo.com/?q={encoded_topic}&iax=images&ia=images"
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                print(f"  Warning: Initial DDG load failed or timed out: {e}")
            ...
            while len(image_urls) < self.max_images and scroll_attempts < max_scrolls:
                ...
                try:
                    # DuckDuckGo image class is often img.tile--img__img
                    # or we can just grab all imgs and filter out logos.
                    images = page.query_selector_all('img')
                    for img in images:
                        src = img.get_attribute('src') or ''
                        if src and src.startswith('//external-content.duckduckgo.com'):
                            src = "https:" + src
                            image_urls.add(src)
        ...
        for url in list(image_urls)[:self.max_images]:
            filepath = self.download_image(url, "duckduckgo")
            if filepath:
                yield filepath, url
